...
public class SpaceManager {
    ...
    public void load() {
        ...
        for (String idStr : cfg.getKeys(false)) {
            try {
                ...
                Set<Space.BlockPos> set = new HashSet<>();
                for (String s : coords) set.add(Space.BlockPos.parse(s));
    ...
    public void save() {
        ...
        for (Space s : spaces.values()) {
            ...
            for (Space.BlockPos p : s.getBlocks()) list.add(p.toString());
    ...
    public void deleteSpace(UUID id) {
        ...
        if (s == null) return;
        // remove from index
        for (Space.BlockPos p : s.getBlocks()) {
    ...
    public void overwriteSpace(Space updated) {
        ...
        if (prev != null) {
            for (Space.BlockPos p : prev.getBlocks()) {
                index.remove(key(prev.getWorldName(), p.x, p.y, p.z));
            }
        }
        for (Space.BlockPos p : updated.getBlocks()) {
    ...
    public Optional<Space> createFromFloodFill(Player player, int blocksPerTick, FloodFillCallback callback) {
        ...
        Bukkit.getScheduler().runTaskTimer(plugin, task -> {
            ...
            while (placed < quota && scanBudget-- > 0) {
                ...
                // collect
                collected.add(new Space.BlockPos(b.getX(), y, b.getZ()));
                placed++;
...
// Only unsealed if the air block is ABOVE the highest solid, not equal
    ...
    public InfluenceResult computeInfluence(World world, Set<Space.BlockPos> airCells) {
        double total = 0.0;
        int air = airCells.size();
        if (air == 0) return new InfluenceResult(0.0, 0.0);

        for (Space.BlockPos p : airCells) {
