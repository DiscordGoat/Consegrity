...
public class CropHarvestListener implements Listener {
    ...
    public void onCropDrop(BlockDropItemEvent event) {
        ...
        switch (type) {
            case CARROTS -> {
                drop(world, dropLocation, new ItemStack(Material.CARROT, random.nextInt(1, 5)), event);
                drop(world, dropLocation, ItemRegistry.getTinyCarrot(), event);
            }
            case POTATOES -> {
                drop(world, dropLocation, new ItemStack(Material.POTATO, random.nextInt(1, 5)), event);
            ...
            case BEETROOTS -> {
                drop(world, dropLocation, new ItemStack(Material.BEETROOT, random.nextInt(1, 5)), event);
