...
public final class CherrySamuraiBehaviour implements Runnable {
    ...
    private void tickCombat(NPC npc, LivingEntity living, SamuraiContext context, LivingEntity target, long now) {
        ...
        float baseSpeed = (float) (deflecting ? DEFLECTION_SPEED_MODIFIER : 1.0);
        float adjustedSpeed = MovementSpeedUtil.applySlowness(living, baseSpeed);
    ...
    private void enterRoamState(NPC npc, LivingEntity living, SamuraiContext context) {
        ...
        npc.getNavigator().cancelNavigation();
        npc.getNavigator().getLocalParameters().speedModifier(MovementSpeedUtil.applySlowness(living, 1.0f));
    ...
    private void stepBackFromTarget(LivingEntity samurai, LivingEntity target) {
        ...
        double multiplier = MovementSpeedUtil.getSlownessMultiplier(samurai);
        if (multiplier <= 0.0) {
            return;
        }
        away.normalize().multiply(0.9 * multiplier);
