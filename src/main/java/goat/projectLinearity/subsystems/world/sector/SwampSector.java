        ...
        for (int lx = 0; lx < 16; lx++) {
            for (int lz = 0; lz < 16; lz++) {
                ...
                if (water) {
                    ...
                    if (ground == Material.DIRT || ground == Material.MUD) {
                        data.setBlock(lx, topY, lz, Material.GRASS_BLOCK);
                        ground = Material.GRASS_BLOCK;
                    } else if (ground != Material.GRASS_BLOCK) {
                        data.setBlock(lx, topY, lz, Material.GRASS_BLOCK);
                        ground = Material.GRASS_BLOCK;
                    }
    ...
    private void growSwampTree(ChunkGenerator.ChunkData data, int lx, int y, int lz, Material log, Material leaves, SplittableRandom rng) {
        ...
        for (int dx = -2; dx <= 2; ++dx) {
            for (int dz = -2; dz <= 2; ++dz) {
                ...
                int radius = (int) Math.ceil(Math.sqrt(r2));
                BlockData lowerLeaves = persistentLeaves(leaves, 1 + radius);
                data.setBlock(x, cy, z, lowerLeaves);
                if (r2 <= 3) {
                    BlockData upperLeaves = persistentLeaves(leaves, 2 + radius);
                    data.setBlock(x, cy + 1, z, upperLeaves);
                }
            }
        }
    }
