...
public class ArcticChunkGenerator extends ChunkGenerator {
    ...
    private static final int CAVE_MIN_Y = -192; // allow caves to reach deep audit band
...
// near top of class
    ...
    private boolean isRegionAnchor(int chunkX, int chunkZ) {
        // Only run on 8Ãƒâ€”8 chunk anchors
                        ...
                        for (int wx = minX; wx <= maxX; wx++) {
                            ...
                            for (int wz = minZ; wz <= maxZ; wz++) {
                                ...
                                for (int wy = minY; wy <= maxY; wy++) {
                                    if (wy <= AUDIT_Y_MIN) continue; // protect bedrock
                                    double dx = wx - x;
                                    double dy = wy - y;
                                    double dz = wz - z;
    ...
    protected void placeOres(World world, ChunkData data, long seed, int chunkX, int chunkZ) {
    ...
    private void generateMountains(World world, ChunkData data, long seed, int chunkX, int chunkZ, BiomeGrid biome) {
        ...
        for (int lx = 0; lx < 16; lx++) {
            for (int lz = 0; lz < 16; lz++) {
                ...
                // Normalize around 0 with amplitude ~Ãƒâ€šÃ‚Â±30..35
                ...
                double ridgeAdd = (ridge - 0.5) * 2.0 * 10.0; // Ãƒâ€šÃ‚Â±10
                ...
                double plains = (fbm2(seed ^ 0x600DD00EL, wx * 0.02, wz * 0.02) - 0.5) * 2.0; // Ãƒâ€šÃ‚Â±1
        ...
        for (int lx = 0; lx < 16; lx++) {
            for (int lz = 0; lz < 16; lz++) {
                ...
                if (!nearShore) {
                    if (Double.isFinite(distPeak) && finalSurf >= 155 && finalSurf <= 200) {
                        ...
                        if (distPeak > rSlope && distPeak <= rHill) {
                            ...
                            int delta = (int) Math.round((n - 0.5) * 20.0 * s); // up to Ã‚Â±10 scaled
                            ...
                            int delta = (int) Math.round((n - 0.5) * 20.0 * (0.5 + 0.5 * s)); // up to Ã‚Â±10
                            finalSurf = Math.max(150, Math.min(worldMaxY, finalSurf + delta));
                        }
                    }
                }
                ...
                if (isOcean) {
                    ...
                    // Edge-touch rule: if any 4-neighbor at sea level is water, make this top Ã¢â€°Â¤ sea level sand
    ...
    protected void carveCavesV2(World world, ChunkData data, long seed, int chunkX, int chunkZ) {
        final int REGION = 64;
        ...
        // World-anchored region replay with conservative reach culling to avoid seams
        // Max horizontal travel for a worm/chaos tunnel (steps + overshoot) * max stepLen + max radius
        final double MAX_TRAVEL = (220 + 99 + 15) * 1.9 + 7.0; // ~640 blocks
        int cellX0 = Math.floorDiv(chunkBaseX, REGION);
        int cellZ0 = Math.floorDiv(chunkBaseZ, REGION);
        int reachCells = (int) Math.ceil(MAX_TRAVEL / REGION) + 1; // ~11

        for (int rx = cellX0 - reachCells; rx <= cellX0 + reachCells; rx++) {
            for (int rz = cellZ0 - reachCells; rz <= cellZ0 + reachCells; rz++) {
                if (!cellMayTouchChunk(rx, rz, REGION, chunkX, chunkZ, MAX_TRAVEL)) continue;
                long rseed = hash(seed, rx, rz, 0, 0x57EEDCAFL);
                Random rr = new Random(rseed);
    ...
    private static boolean cellMayTouchChunk(int cx, int cz, int cellSize, int chunkX, int chunkZ, double reach) {
        ...
        double dx = (cellMaxX < chMinX) ? (chMinX - cellMaxX) : (cellMinX > chMaxX ? cellMinX - chMaxX : 0.0);
        double dz = (cellMaxZ < chMinZ) ? (chMinZ - cellMaxZ) : (cellMinZ > chMaxZ ? cellMinZ - chMaxZ : 0.0);
