...
public class VanillaAdvancementDisablerWrapper_v1_20_R4
extends VanillaAdvancementDisablerWrapper {
    ...
    public static void disableVanillaAdvancements(boolean vanillaAdvancements, boolean vanillaRecipeAdvancements) throws Exception {
        AdvancementDataWorld serverAdvancements = ((CraftServer)Bukkit.getServer()).getServer().aE();
        AdvancementTree tree = serverAdvancements.a();
        if (serverAdvancements.c.isEmpty()) {
            return;
        }
        final HashSet removed = Sets.newHashSetWithExpectedSize((int)serverAdvancements.c.size());
        ...
        try {
            ...
            for (Map.Entry entry : serverAdvancements.c.entrySet()) {
            ...
            serverAdvancements.c = builder.buildOrThrow();
            Level oldLevel = VanillaAdvancementDisablerWrapper_v1_20_R4.disableLogger();
            try {
                tree.a(locations);
            }
            finally {
                VanillaAdvancementDisablerWrapper_v1_20_R4.enableLogger(oldLevel);
            }
        }
        finally {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            AdvancementDataPlayer advs = mcPlayer.Q();
            advs.a(serverAdvancements);
            firstPacket.setBoolean(advs, false);
            mcPlayer.c.b((Packet)removePacket);
        }
    }
