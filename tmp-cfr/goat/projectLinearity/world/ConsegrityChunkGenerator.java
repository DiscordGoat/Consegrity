...
extends ArcticChunkGenerator {
        ...
        for (lx = 0; lx < 16; ++lx) {
            for (lz = 0; lz < 16; ++lz) {
                ...
                if (union > 0.03 || C.r <= 150.0) {
                    ...
                    landH = ConsegrityChunkGenerator.lerp(landH, 152.0, sIn);
                    if (d < 152.0) {
                        landH = 152.0;
                    }
                    target = landH;
                } else if (R1.r < R1.innerGate) {
                    t = ConsegrityChunkGenerator.clamp01((R1.r - C.outerGate) / Math.max(1.0, R1.innerGate - C.outerGate));
                    t = t * t * (t * -2.0 + 3.0);
                ...
                if (r1m >= r2m) {
                    if (b1.a != b1.b) {
                        t = ConsegrityChunkGenerator.smooth01(1.0 - b1.t);
                        int left = b1.a;
                        right = b1.b;
                        if (left == 0 && right != 0 || right == 0 && left != 0) {
                            tLand = right != 0 ? t : 1.0 - t;
                            this.paintDesertLandBlendCap(data, lx, lz, topY, tLand);
                        } else {
                            this.paintLandCap(data, lx, lz, topY);
                        }
                        idx = t >= 0.5 ? left : right;
                        int n = idx;
                        if (idx == 0) {
                ...
                if (b2.a != b2.b) {
                    t = ConsegrityChunkGenerator.smooth01(1.0 - b2.t);
                    int left = b2.a;
                    right = b2.b;
                    if (left == 0 && right != 0 || right == 0 && left != 0) {
                        tLand = right != 0 ? t : 1.0 - t;
    ...
    private void placeSavannaFeatures(World world, ChunkGenerator.ChunkData data, long seed, int chunkX, int chunkZ, int[][] topYGrid, ConsegrityRegions.Region[][] regionGrid) {
        ...
        int lz;
        int lx;
        SplittableRandom rng = ConsegrityChunkGenerator.rngFor(seed, chunkX, chunkZ, 0x9090919L);
        for (int i = 0; i < 4; ++i) {
            int ox;
            lx = rng.nextInt(16);
            lz = rng.nextInt(16);
            if (lx < 1 || lx > 14 || lz < 1 || lz > 14 || regionGrid[lx][lz] != ConsegrityRegions.Region.SAVANNAH || (topY = topYGrid[lx][lz]) < 155 || this.slopeGrid(topYGrid, lx, lz) > 3 || (ground = this.safeType(data, lx, topY, lz)) != Material.GRASS_BLOCK && ground != Material.DIRT || rng.nextDouble() > 0.35) continue;
            int height = 4 + rng.nextInt(3);
            int dir = rng.nextInt(4);
            int n = dir == 0 ? 1 : (ox = dir == 1 ? -1 : 0);
            ...
            for (int dx = -r; dx <= r; ++dx) {
                for (int dz = -r; dz <= r; ++dz) {
                    ...
                    if (!(rng.nextDouble() < 0.35) || this.safeType(data, xx, canopyY + 1, zz) != Material.AIR) continue;
                    data.setBlock(xx, canopyY + 1, zz, Material.ACACIA_LEAVES);
                }
            }
        }
        Material grassPlant = this.pickGrassPlant();
        for (lx = 0; lx < 16; ++lx) {
            for (lz = 0; lz < 16; ++lz) {
                if (regionGrid[lx][lz] != ConsegrityRegions.Region.SAVANNAH) continue;
                topY = topYGrid[lx][lz];
                ground = this.safeType(data, lx, topY, lz);
                Material above = this.safeType(data, lx, topY + 1, lz);
                if (ground != Material.GRASS_BLOCK && ground != Material.DIRT || above != Material.AIR || !(rng.nextDouble() < 0.4)) continue;
    ...
    private void placeCentralGrass(World world, ChunkGenerator.ChunkData data, long seed, int chunkX, int chunkZ, int[][] topYGrid, double[][] centralMaskGrid) {
        ...
        for (int lx = 0; lx < 16; ++lx) {
            for (int lz = 0; lz < 16; ++lz) {
                int topY;
                if (centralMaskGrid[lx][lz] < 0.5 || (topY = topYGrid[lx][lz]) <= 157) continue;
                Material ground = this.safeType(data, lx, topY, lz);
                Material above = this.safeType(data, lx, topY + 1, lz);
                if (ground != Material.GRASS_BLOCK && ground != Material.DIRT || above != Material.AIR || !(rng.nextDouble() < 0.08)) continue;
                data.setBlock(lx, topY + 1, lz, grassPlant);
            }
        }
    }
    ...
    private Material pickGrassPlant() {
        try {
            return Material.valueOf((String)"SHORT_GRASS");
        }
        catch (Throwable ignore) {
            try {
                return Material.valueOf((String)"TALL_GRASS");
            }
            catch (Throwable ignore2) {
                try {
                    return Material.valueOf((String)"GRASS");
                }
    ...
    private void placeSwampFeatures(World world, ChunkGenerator.ChunkData data, long seed, int chunkX, int chunkZ, int[][] topYGrid, ConsegrityRegions.Region[][] regionGrid) {
        ...
        int baseZ = chunkZ << 4;
        try {
            lily = Material.valueOf((String)"LILY_PAD");
        }
        catch (Throwable t) {
            lily = Material.valueOf((String)"WATER_LILY");
        }
        double waterFrac = 0.4;
        double scale = 34.0;
        for (int lx = 0; lx < 16; ++lx) {
            for (int lz = 0; lz < 16; ++lz) {
                boolean water;
                ...
                int wx = baseX + lx;
                int wz = baseZ + lz;
                double n = ConsegrityChunkGenerator.valueNoise2(seed ^ 0x5A4D00A1L, (double)wx / 34.0, (double)wz / 34.0);
                boolean bl = water = n < 0.4;
                if (water) {
                    data.setBlock(lx, topY, lz, Material.WATER);
                    if (!(rng.nextDouble() < 0.1) || this.safeType(data, lx, topY + 1, lz) != Material.AIR) continue;
                    try {
                        data.setBlock(lx, topY + 1, lz, lily);
                    }
                    catch (Throwable throwable) {}
                    continue;
                }
                Material g = this.safeType(data, lx, topY, lz);
                if (g == Material.GRASS_BLOCK || g == Material.DIRT || g == Material.MUD) continue;
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            int lz;
            int lx = rng.nextInt(16);
            if (regionGrid[lx][lz = rng.nextInt(16)] != ConsegrityRegions.Region.SWAMP) continue;
            int topY = topYGrid[lx][lz];
            int wx = baseX + lx;
            int wz = baseZ + lz;
            double n = ConsegrityChunkGenerator.valueNoise2(seed ^ 0x5A4D00A1L, (double)wx / 34.0, (double)wz / 34.0);
            if (n < 0.4 || this.slopeGrid(topYGrid, lx, lz) > 3 || (ground = this.safeType(data, lx, topY, lz)) != Material.GRASS_BLOCK && ground != Material.DIRT || rng.nextDouble() > 0.18) continue;
            ...
            this.addVinesAround(data, lx, topY + 1, lz, rng);
        }
    }

    private void addVinesAround(ChunkGenerator.ChunkData data, int x, int y, int z, SplittableRandom rng) {
        try {
            block6: for (int side = 0; side < 4; ++side) {
                int dz;
                int dx;
                if (rng.nextDouble() > 0.6) continue;
                int n = side == 0 ? 1 : (dx = side == 1 ? -1 : 0);
                int n2 = side == 2 ? 1 : (dz = side == 3 ? -1 : 0);
                ...
                for (int k = 0; k < len; ++k) {
                    int xx = x + dx;
                    int zz = z + dz;
                    if (xx < 0 || xx > 15 || zz < 0 || zz > 15) continue block6;
                    if (this.safeType(data, xx, yy - k, zz) != Material.AIR) continue;
                    BlockData bd = Bukkit.createBlockData((Material)Material.VINE);
                    try {
                    ...
                    try {
                        data.setBlock(xx, yy - k, zz, bd);
                        continue;
                    }
                    catch (Throwable t) {
                        data.setBlock(xx, yy - k, zz, Material.VINE);
                    }
                }
            }
        }
    ...
    private int ring2MountainSurfaceY(long seed, int wx, int wz) {
        ...
        double d2 = Math.sqrt(ddx * ddx + ddz * ddz);
        double R = 72.0;
        double w = ConsegrityChunkGenerator.clamp01(1.0 - d2 / R);
        ...
        h = base + (h - base) * (1.35 * cellVar);
        if (d > (double)worldMaxY) {
            h = worldMaxY;
        }
        if (h < 150.0) {
            h = 150.0;
        ...
        for (int i = 0; i < centers; ++i) {
            ...
            for (int dx = -r - 1; dx <= r + 1; ++dx) {
                for (int dz = -r - 1; dz <= r + 1; ++dz) {
                    ...
                    int x = cx + dx;
                    int z = cz + dz;
                    if (x < 0 || x > 15 || z < 0 || z > 15 || !frozenOcean[x][z]) continue;
                    double d2 = Math.hypot(dx, dz);
                    if (d > (double)r + 0.8) continue;
                    double t = Math.max(0.0, 1.0 - d2 / ((double)r + 0.8));
    ...
    private static double[] sectorWeightsEqual(double u, int sectors, double blendFrac) {
        ...
        double t = v - (double)k;
        double b = Math.max(1.0E-4, blendFrac);
        if (t < d) {
    ...
    private static Blend angularTwoWay(double u, int sectors, double blendFrac) {
        ...
        int k = (int)Math.floor(v);
        double t = v - (double)k;
        double b = Math.max(1.0E-6, blendFrac);
        if (t < d) {
            double s = ConsegrityChunkGenerator.smooth01(t / b);
            ...
            for (int cz = cellZ0 - CAVE_REACH; cz <= cellZ0 + CAVE_REACH; ++cz) {
                ...
                block2: for (int i = 0; i < spawns; ++i) {
                    ...
                    for (int s = 0; s < steps; ++s) {
                        ...
                        pitch += (rr.nextDouble() - 0.5) * 0.2 - 0.035;
                        if (d < -0.9) {
                            pitch = -0.9;
                        }
                        if (pitch > 0.9) {
